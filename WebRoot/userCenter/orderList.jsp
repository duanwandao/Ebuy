					...
					<c:forEach items="${orderList }" var="order">
						<tr style="background-color: #f7f4eb">
							<td colspan="4">
								...
								<c:choose>
									...
									<c:when test="${order.status==3 }"><input type="button" value="确实收获" style="border:1px solid red" onclick="confirmReceive(${order.orderNo})"/></c:when>
						...
						<c:forEach items="${order.orderProductList }" var="orderProduct">
							<tr>
								...
								<td>&nbsp;&nbsp;小计：
									${orderProduct.num*orderProduct.product.price}&nbsp;(元)</td>
							</tr>
						</c:forEach>
					</c:forEach>
				</table>
